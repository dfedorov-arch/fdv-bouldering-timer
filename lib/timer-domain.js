...
function runningElapsedAfterRestore(
  ...
  if (
    ...
    return fallbackElapsed + (safeCurrentUptime - safeSavedUptime) / 1000;
  }
  const snapshotAgeSeconds = Number.isFinite(safeNow) && Number.isFinite(safeSavedAt)
    ? Math.max(0, safeNow - safeSavedAt) / 1000
    : 0;
  if (Number.isFinite(safeStart) && safeStart > 0 && Number.isFinite(safeNow)) {
    return Math.max(0, (safeNow - safeStart) / 1000);
  }
