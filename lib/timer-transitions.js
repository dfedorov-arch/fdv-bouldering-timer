...
function createTimerTransitions(domain, options = {}) {
  ...
  function applyTimerAction(state, action = {}, context = {}) {
    ...
    if (type === "start") {
      ...
      const resumesExistingRun = !state.completed && (pausedElapsed > 0 || state.waitingForManualStart);
      ...
      if (!resumesExistingRun) {
        nextState.runtimePreset = Object.prototype.hasOwnProperty.call(action, "activePreset")
          ? action.activePreset || ""
          : state.activePreset || "";
      }
      ...
      if (!state.running && state.elapsedBeforePause === 0) {
        nextState.activeSettings = domain.normalizeActiveSettings({
          ...
          finalRoundFormat: nextState.draftSettings.finalRoundFormat,
          finalRestRotations: nextState.draftSettings.finalRestRotations
        }, state.activeSettings);
        nextState.runtimePreset = nextState.activePreset;
      }
      changed = true;
    }
