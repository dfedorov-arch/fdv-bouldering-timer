...
В готовом Windows-релизе запускайте fdv-bouldering-timer.exe. Файл
start-timer-win.bat остаётся резервным способом запуска.
...
In a ready-to-use Windows release, run fdv-bouldering-timer.exe.
start-timer-win.bat remains available as a fallback launcher.
